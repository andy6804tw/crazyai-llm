...
在 Dify 的工作室中點擊「建立新工作流」，命名為「**網頁爬蟲**」。
...
| 節點                 | 功能     | 說明                                          |
| ------------------ | ------ | ------------------------------------------- |
| **開始 (Start)**     | 定義輸入參數 | 新增一個文字欄位 `weburl` (必填)                      |
| **Web Scraper 工具** | 進行網頁擷取 | 工具類型：`webscraper`，參數來源：`{{#start.weburl#}}` |
| **結束 (End)**       | 定義輸出結果 | 輸出變數：`text`，取自 `Web Scraper.text`           |
...
設定完成後，點擊「儲存」，Dify 便會將你的工作流正式封裝為 MCP Server，供後續在 Agent 或其他平台中掛載使用。
...
1. 進入左側選單的 **工具 (Tools)** → 點選上方分頁 **MCP**。
2. 點擊「**新增 MCP 伺服器 (HTTP)**」。
3. 填寫欄位：

   * **伺服器 URL**：貼上剛剛的 MCP Server URL（例如 `http://10.136.216.100:7030/...`）
   * **名稱與顯示**：爬蟲器
   * **伺服器識別碼**：`crawler`（僅限小寫英數字）
...
在左側選單中選擇「Agent」，點擊右上角「建立新 Agent」，為這個智能體命名（例如「網頁摘要助理」）。

### 2. 選擇模型
可選擇任一支援工具調用的模型，例如 `openai/gpt-4o` 或 `qwen-3`。若使用企業部署環境，也可選擇內部部署的 LLM。
...
你是一個資訊擷取與摘要助理。
當使用者提供網址時：
- 使用工具「爬蟲器」呼叫對應 MCP 伺服器，參數 { weburl: <使用者網址> }。
- 取得網頁文字後，進行摘要、整理重點。
- 回覆使用者主要段落與摘要。
```
...
* 偵測使用者輸入中是否包含 URL；
* 若有，透過 MCP 工具「爬蟲器」進行抓取；
* 最後由模型生成一段條理清晰的摘要回覆。

### 4. 掛上 MCP 工具
在下方的「工具」區塊中，找到剛剛建立的 **爬蟲器 (MCP)**，點擊啟用（開關切換為藍色「啟用中」狀態）。
